...
from filevine import ConnectionManager, RateLimiter, TokenManager, ModelFactory, FilevineHTTPException, FilevineRateLimitException
...
class TestConnectionManager(unittest.TestCase):
    @patch('filevine.connection_manager.TokenManager', MockTokenManager)
    ...
    def test_handle_response(self):
        async def test():
            ...
            with self.assertRaises(FilevineHTTPException):
                response = MockResponse(500)
                await conn._ConnectionManager__handle_response(response, 200)
            # test 429 Rate Limit error
            with self.assertRaises(FilevineRateLimitException):
    ...
    @patch('filevine.connection_manager.TokenManager', MockTokenManager)
    @patch('filevine.connection_manager.RateLimiter', autospec=True)
